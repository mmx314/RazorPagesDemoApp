...
namespace RazorPagesDemoApp.Data
{
	...
	//Класс RazorPagesDemoDbContext отвечает за взаимодействие с базой данных и
	//предоставляет доступ к данным в виде объектов DbSet.
	//Этот класс содержит сведения о том, как сопоставить модель данных(Employee) с таблицей в базе данных,
	//а также методы для выполнения операций CRUD(Create, Read, Update, Delete) над этой таблицей.
	//Конструктор RazorPagesDemoDbContext принимает параметры, необходимые для установления соединения с базой данных,
	//такие как строка подключения и провайдер базы данных.

	//5. При использовании данного контекста данных мы можем получить доступ к сотрудникам через свойство Employees,
	//например: dbContext.Employees
